...
export class AuthController {
  ...
  async login(
    ...
    const userSettings = await this.userSettingsModel
      .findOne({ _id: findedUser.settings })
      .exec();
  ...
  async OAouth(
    ...
    const findedUser = await this.userModel
      .findOne({ email: oAuth.email })
      .populate('role settings')
      .lean()
      .exec();

    if (!findedUser) {
      const settings = new this.userSettingsModel({
        emailPhoto: true,
        statusOnline: true,
      });
      await settings.save();
      const roleDocument = await this.roleModel
        .findOne({ role: UserRoles.user })
        .exec();
      if (!roleDocument) {
        throw new NotFoundException('no role like that');
      }
