...
export class TaskService extends BaseService<Task> {
  constructor(
    ...
    @InjectModel(StatusOfTask.name)
    private statusOfTaskModel: Model<StatusOfTask>,
  ...
  async createTask({ assignPerson, ...createTaskDto }: CreateTaskDto) {
    const newStatusTask = await this.statusOfTaskModel
      .findOne({ status: TaskStatus.TODO })
      .exec();
    if (newStatusTask) {
  ...
  async changeStatusByName(taskId: string, newStatusName: string) {
    ...
    // Récupérer le statut correspondant à l'ID donné
    const newStatus = await this.statusOfTaskModel.findOne({
